...
class Auth {
    ...
    public function getGroups($uid) {
        ...
        $groupsCachedData = $cache->get($groupsCacheID);
        //查看文件中是否有缓存
        if (!empty($groupsCachedData)) {
            ...
            ////设置文件缓存
            $cache->set($groupsCacheID, $userGroups);
            //设内存缓存
    ...
    public static function cache() {
        if (self::$cache == null) {
            self::$cache = new \engine\cache\SimpleFileCache();
            self::$cache->setCacheDir();
        }
        return self::$cache;
    }
